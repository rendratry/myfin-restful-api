...
func (repository *PengajuanKreditRepositoryImpl) AjuanKredit(ctx context.Context, tx *sql.Tx, ajuankredit domain.PengajuanKredit) domain.PengajuanKredit {
	script := "insert into tb_pengajuan_kredit(id_nasabah,penggunaan,tanggal_pengajuan, besar_pengajuan, tenor, score, status) values (?,?,?,?,?,?,?)"
	result, err := tx.ExecContext(ctx, script, ajuankredit.IdNasabah, ajuankredit.Penggunaan, ajuankredit.TanggalPengajuan, ajuankredit.BesarPengajuan, ajuankredit.Tenor, ajuankredit.Score, ajuankredit.Status)
